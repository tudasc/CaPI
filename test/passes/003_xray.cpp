...
// RUN: %clang_cxx -v -g -fxray-instrument -Wl,--whole-archive -L %lib_dir/xray -l %capi_xray_lib -Wl,--no-whole-archive %s.o -L %lib_dir/calltree_logger -lcalltreelogger_static `%llvm-config --libfiles xray symbolize --link-static --system-libs` -o %s.exe
// RUN: CAPI_EXE=$(basename %s.exe) CAPI_OPTIONS="--enable" XRAY_OPTIONS="patch_premain=false verbosity=1" %s.exe
// RUN: cat %s.exe.capi.log | FileCheck %s
// RUN: rm %s.exe.capi.log
// RUN: CAPI_EXE=$(basename %s.exe) CAPI_OPTIONS="--filter-file=003_xray.filt" XRAY_OPTIONS="patch_premain=false verbosity=1" %s.exe
