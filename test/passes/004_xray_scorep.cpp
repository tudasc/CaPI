...
// RUN: %clang_cxx -fuse-ld=lld -v -g -fxray-instrument -L `scorep-config --prefix`/lib -Wl,-start-group -Wl,--whole-archive -L %lib_dir/xray -l %capi_xray_scorep_lib -Wl,--no-whole-archive `scorep-config --libs --mpp=none --compiler` `%llvm-config --libfiles xray symbolize --link-static --system-libs` -Wl,-end-group  scorep_init.o %s.o -o %s.exe
