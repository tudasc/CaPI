...
//
// RUN: infile="%s"; %capi -i 'by_name("main") |> on_call_path_from' -o %s_down.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_down.filt | c++filt | sort | %filecheck %s -check-prefix=DOWN
//
// RUN: infile="%s"; %capi -i 'by_name("C::foo") |> on_call_path_to' -o %s_up.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_up.filt | c++filt | sort | %filecheck %s -check-prefix=UP
//
// RUN: infile="%s"; %capi -i 'by_name("@_ZN1C3fooEv") |> on_call_path_to' -o %s_up.filt --output-format simple ${infile%%.*}.ipcg
