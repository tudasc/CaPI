...
//
// RUN: infile="%s"; timeout 5s %capi -i '[by_name("c1"), by_name("c2")] |> common_caller' -o %s.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s.filt | c++filt | sort | %filecheck %s
//
// RUN: infile="%s"; %capi -i '[by_name("c1"), by_name("c2")] |> common_caller_distinct' -o %s.filt --output-format simple ${infile%%.*}.ipcg
