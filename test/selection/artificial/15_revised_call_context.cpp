...
//
// RUN: infile="%s"; timeout 10s %capi -i '[by_name("x"), by_name("y")] |> common_caller(0)' -o %s_dist0.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_dist0.filt | c++filt | sort | %filecheck -check-prefix=MAX-DIST0 %s
//
// RUN: infile="%s"; timeout 10s %capi -i '[by_name("x"), by_name("y")] |> common_caller(1)' -o %s_dist1.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_dist1.filt | c++filt | sort | %filecheck -check-prefix=MAX-DIST1 %s
//
// RUN: infile="%s"; timeout 10s %capi -i '[by_name("x"), by_name("y")] |> common_caller(2)' -o %s_dist2.filt --output-format simple ${infile%%.*}.ipcg
