...
//
// RUN: infile="%s"; timeout 10s %capi -i '(%%%% | by_name("b"), %%%% | by_name("d")) | common_caller' -o %s.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s.filt | c++filt | sort | %filecheck %s
//
// RUN: infile="%s"; timeout 10s %capi -i '(%%%% | by_name("b"), %%%% | by_name("d")) | common_caller(1)' -o %s_dist1.filt --output-format simple ${infile%%.*}.ipcg
