...
//
// RUN: infile="%s"; %capi -i 'by_name("foo") |> on_call_path_from' -o %s_down.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_down.filt | c++filt | sort | %filecheck %s -check-prefix=DOWN
//
// RUN: infile="%s"; %capi -i 'by_name("foo") |> on_call_path_from' -o %s_down_v.filt --output-format simple --traverse-virtual-dtors ${infile%%.*}.ipcg
// RUN: cat %s_down_v.filt | c++filt | sort | %filecheck %s -check-prefix=DOWN-VIRTUAL
//
// RUN: infile="%s"; %capi -i 'by_name("@_ZN1CD0Ev") |> on_call_path_to' -o %s_up.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_up.filt | c++filt | sort | %filecheck %s -check-prefix=UP
//
// RUN: infile="%s"; %capi -i 'by_name("@_ZN1CD0Ev") |> on_call_path_to' -o %s_up_v.filt --output-format simple --traverse-virtual-dtors ${infile%%.*}.ipcg
