...
//
// RUN: infile="%s"; timeout 10s %capi -i 'common_caller(0, byName("t1", %%%%), byName("t2", %%%%))' -o %s_dist0.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_dist0.filt | c++filt | sort | %filecheck -check-prefix=MAX-DIST0 %s
//
// RUN: infile="%s"; timeout 10s %capi -i 'common_caller(1, byName("t1", %%%%), byName("t2", %%%%))' -o %s_dist1.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_dist1.filt | c++filt | sort | %filecheck -check-prefix=MAX-DIST1 %s
//
// RUN: infile="%s"; timeout 10s %capi -i 'common_caller(2, byName("t1", %%%%), byName("t2", %%%%))' -o %s_dist2.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_dist2.filt | c++filt | sort | %filecheck -check-prefix=MAX-DIST2 %s
//
// RUN: infile="%s"; timeout 10s %capi -i 'common_caller(3, byName("t1", %%%%), byName("t2", %%%%))' -o %s_dist3.filt --output-format simple ${infile%%.*}.ipcg
