...
//
// RUN: infile="%s"; %capi -i '%%%% | by_name("a") | on_call_path_to' -o %s_callers_a.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_callers_a.filt | c++filt | sort | %filecheck %s -check-prefix=CALLERS-A
//
// RUN: infile="%s"; %capi -i '%%%% | by_name("f") | on_call_path_to' -o %s_callers_f.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_callers_f.filt | c++filt | sort | %filecheck %s -check-prefix=CALLERS-F
//
// RUN: infile="%s"; %capi -i '%%%% | by_name("a") | on_call_path_from' -o %s_callees_a.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_callees_a.filt | c++filt | sort | %filecheck %s -check-prefix=CALLEES-A
//
// RUN: infile="%s"; %capi -i '%%%% | by_name("f") | on_call_path_from' -o %s_callees_f.filt --output-format simple ${infile%%.*}.ipcg
