...
//
// RUN: infile="%s"; %capi -i 'sel=byName("(main)|(a)", %%%%) !instrument(%%sel)' -o %s_inst.filt --output-format simple ${infile%%.*}.ipcg
// RUN: cat %s_inst.filt | c++filt | sort | %filecheck %s -check-prefix=INST
//
// RUN: infile="%s"; %capi -i 'start=byName(".*c.*", %%%%) sel=byName("(main)|(a)", %%%%) !instrument(%%sel) !begin_after(%%start)' -o %s_inst2.filt --output-format simple ${infile%%.*}.ipcg
