...
extern "C" LLVM_ATTRIBUTE_WEAK PassPluginLibraryInfo llvmGetPassPluginInfo() {
  return {
      LLVM_PLUGIN_API_VERSION, "talp-inst", "v0.1", [](llvm::PassBuilder& PB) {
        PB.registerOptimizerLastEPCallback([&](ModulePassManager &MPM, OptimizationLevel O, ThinOrFullLTOPhase) {
          MPM.addPass(createModuleToFunctionPassAdaptor(TALPInstrumenter()));
          return true;
        });
      }
  };
...
static StructType* getMonitorStructType(Module& M) {
  auto MonitorTy = StructType::getTypeByName(M.getContext(), "dlb_monitor_t");
  if (MonitorTy) {
    return MonitorTy;
  }
...
static Value* insertRegionEnter(Function &F, Instruction* InsertionPt, DebugLoc DL) {
  ...
  FunctionCallee RegisterFn = M.getOrInsertFunction("DLB_MonitoringRegionRegister", MonitorPtrTy, PointerType::get(C, 0));
  ...
  auto RegionName = IRB.CreateGlobalString(F.getName());
  auto RegionNamePtr = IRB.CreateInBoundsGEP(IRB.getInt8Ty(), RegionName, ConstantInt::get(IRB.getInt64Ty(), 0, false));
...
TALPInstrumenter::TALPInstrumenter() {
}

llvm::PreservedAnalyses TALPInstrumenter::run(llvm::Function &F, llvm::FunctionAnalysisManager &) {
