...
bool MetaCGReader::read() {
  ...
  for (auto it = jsonCG.begin(); it != jsonCG.end(); ++it) {
    ...
    //        if (!jCallees.is_null()) {
    //            std::for_each(jCallees.begin(), jCallees.end(), [&fi](auto&
    //            jCallee) {
    ...
    if (!jMeta.is_null()) {
      ...
      if (jInstrument.is_boolean()) {
        ...
        llvm::outs() << "Instrumentation not specified for function "
                     << it.key() << ": not instrumenting.\n";
      }
    } else {
      llvm::outs() << "No metadata for function " << it.key()
                   << ": not instrumenting.\n";
    }
  }
  return true;
