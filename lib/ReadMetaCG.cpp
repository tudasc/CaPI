...
namespace {
    static llvm::RegisterPass<ReadMetaCG> passreg("readmetacg", "Read MetaCG database from file", false, false);
}

static cl::opt<std::string> ClCGFile("metacg-file", cl::desc("Location of the MetaCG file"), cl::Hidden, cl::init("cg.ipcg"));

static cl::opt<std::string> ClCfgFile("inst-config", cl::desc("Location of instrumentation config file"), cl::Hidden, cl::init("ic.json"));
...
//    bool changed = llvm::count_if(m.functions(), [&](auto& f) { errs() << f.getName(); return runOnFunc(f); }) > 0;
//
//    return changed;
}
...
bool ReadMetaCG::doFinalization(llvm::Module &) {
    return false;
}
