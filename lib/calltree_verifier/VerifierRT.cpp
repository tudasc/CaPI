...
std::vector<MemMapEntry> read_memory_map() {
  ...
  while (fgets(buffer, sizeof(buffer), memory_map)) {
    ...
    lineStream >> addrRange >> perms >> std::hex >> offset >> std::dec >> dev >>
        inode >> path;
    // std::cout << "Memory Map: " << addrRange << ", " << perms << ", " <<
    // offset << ", " << path <<"\n";
    ...
    uintptr_t addrBegin =
        std::stoul(addrRange.substr(0, addrRange.find('-')), nullptr, 16);
...
SymbolTable loadSymbolTable(const std::string &object_file) {
  // Need to disable LD_PRELOAD, otherwise this library will be loaded in popen
  // call.
  RemoveEnvInScope removePreload("LD_PRELOAD");
...
std::string resolve_symbol_name(const char *object_file, const void *addr) {
  ...
  char command[256];
  int n =
      sprintf(command, R"(nm -D %s | grep %lx | awk '{ printf "%%s", $3 }')",
              object_file, reinterpret_cast<std::uintptr_t>(addr));

  if (n >= sizeof(command) - 1) {
    std::cerr
        << "Insufficient buffer size for symbol name resolution command.\n";
    return "";
  }
...
RTInitializer::RTInitializer() {
  ...
  if (auto val = getenv("CAPI_EXE"); val) {
    if (strcmp(val, execFilename.c_str()) != 0) {
      return;
    }
    std::cout << "Executable will be tracked: CAPI_EXE is set to "
              << val << " and matches the current executable filename "
              << execFilename << "\n";
  } else {
    std::cout << "Please set CAPI_EXE=" << execFilename
              << ", if you want to track calls in this application.\n";
    return;
  }
  std::cout << "Retrieving symbols for executable " << exec_path << "\n";
  //print_process_map();
  name_cache = std::make_unique<FunctionNameCache>(exec_path);
  auto logFile = execFilename + ".capi.log";
  logger = std::make_unique<CallTreeLogger>(logFile, *name_cache);
}

FunctionNameCache::FunctionNameCache(std::string execFile)
    : execFile(execFile) {
...
void FunctionNameCache::loadSymTables() {
  ...
  //            continue;
  //        }
  //        std::cout << "Loaded " << table.size() << " symbols from " <<
  //        filename << "\n"; symTables.emplace_back(filename,
  //        std::move(table));
  //    }
}
...
std::string FunctionNameCache::resolve(const void *addr) {
  ...
  if (it != addrToSymTable.begin()) {
    ...
    //        std::cout << "Start: " << mapping.addrBegin << ", offset: " <<
    //        mapping.offset <<"\n";
    auto mappedAddr = (addrVal - mapping.addrBegin) + mapping.offset;
    //        std::cout << "Looking for mapped adddress " << mappedAddr << " in
    //        " << mapping.path << "\n";
...
CallTreeLogger::CallTreeLogger(std::string filename, FunctionNameCache &cache)
    : out(std::ofstream(filename)), cache(cache) {
